...
public class PhraseUtilities {
    ...
    static int countSyllablesInWord( String word ) {
        ...
        for ( int i = 0; i < chars.length; i++ ) {
            char c = chars[i];
            if ( isVowel( c ) ) {
                if ( !lastWasVowel || ( i > 0 && isE( chars, i - 1 ) && isO( chars, i ) ) ) {
                    ++syllables;
                    lastWasVowel = true;
                }
            } else {
        ...
        if ( word.endsWith( "oned" ) || word.endsWith( "ne" ) || word.endsWith( "ide" ) || word.endsWith( "ve" ) ||
             word.endsWith( "fe" ) || word.endsWith( "nes" ) || word.endsWith( "mes" ) ) {
            --syllables;
        }
